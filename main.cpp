...
int PIN_LORA_MISO = 23;	 // LORA SPI MISO > SDO
int PIN_LORA_MOSI = 19;	 // LORA SPI MOSI > SDI
int PIN_LORA_BUSY = 32;	 // LORA SPI BUSY 
int PIN_LORA_DIO_1 = 33; // LORA DIO_1
int RADIO_RXEN = 17;	 // LORA ANTENNA RX ENABLE
int RADIO_TXEN = 16;	 // LORA ANTENNA TX ENABLE 
#endif

// Define Helium or TTN OTAA keys. All msb (big endian).
uint8_t nodeDeviceEUI[8] = {/*MSB*/};
uint8_t nodeAppEUI[8] = {/*MSB*/};
uint8_t nodeAppKey[16] = {/*MSB*/};
 ...
 */
static lmh_param_t lora_param_init = {LORAWAN_ADR_OFF, DR_5, LORAWAN_PUBLIC_NETWORK,
...
void setup()
{
	...
	err_code = lmh_init(&lora_callbacks, lora_param_init, doOTAA, CLASS_C, LORAMAC_REGION_EU868);
	if (err_code != 0)
	{
